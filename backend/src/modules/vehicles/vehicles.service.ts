...
export class VehiclesService {
  constructor(
    ...
    private presetScheduleService: PresetScheduleService,
  ) {}
  ...
  async create(userId: number, dto: CreateVehicleDto) {
    ...
    // 3. Khởi tạo tự động bộ lịch bảo dưỡng mẫu nếu autoGenerateSchedules khác false
    let presetInfo: any = null;
    if (dto.autoGenerateSchedules !== false) {
      presetInfo = await this.presetScheduleService.generateSchedulesForVehicle(
        newVehicleId,
        dto.vehicleType,
        dto.currentOdometer,
        dto.purchaseDate,
    ...
    return {
      ...
      autoGeneratedSchedules: presetInfo,
    };
  }
  ...
  // Xem trước danh sách mốc km và bộ lịch mẫu sẽ được sinh tự động
  previewPresetSchedules(vehicleType: string, currentOdometer: number, purchaseDate?: string) {
    return this.presetScheduleService.previewPresetSchedules(vehicleType, currentOdometer, purchaseDate);
  }

  // Lấy danh sách toàn bộ lịch nhắc mẫu vừa khởi tạo cho phương tiện
  async getGeneratedSchedules(id: number, userId: number, role: string) {
    await this.findOne(id, userId, role);

    const schedules = await this.dbService.query(
      `SELECT * FROM MaintenanceSchedules WHERE VehicleID = @id ORDER BY TargetOdometer ASC, TargetDate ASC`,
      [{ name: 'id', type: sql.Int, value: id }]
    );

    const legalDocs = await this.dbService.query(
      `SELECT * FROM LegalDocuments WHERE VehicleID = @id ORDER BY ExpiryDate ASC`,
      [{ name: 'id', type: sql.Int, value: id }]
    );

    return {
      vehicleId: id,
      maintenanceSchedules: schedules.recordset,
      legalDocuments: legalDocs.recordset,
    };
  }
}
