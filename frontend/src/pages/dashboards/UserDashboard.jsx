      ...
      <header className="bg-white dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700 shadow-sm px-6 py-4 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-indigo-650 flex items-center justify-center text-white font-bold text-lg">
              A
            </div>
            <div>
          ...
          <div className="flex items-center gap-4">
            <span className="text-sm text-slate-650 dark:text-slate-350 hidden sm:inline">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-6 py-8">
        ...
        <div className="mb-8 flex flex-col sm:flex-row justify-between sm:items-center gap-4 bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 p-8 shadow-sm">
          ...
          <button
            ...
            className="px-5 py-3 rounded-2xl text-sm font-bold text-white bg-indigo-650 hover:bg-indigo-700 shadow-md hover:shadow-lg transition flex items-center justify-center gap-2 self-start sm:self-auto"
          >
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[1, 2].map((i) => (
              <div key={i} className="h-48 rounded-3xl bg-slate-200 dark:bg-slate-800 animate-pulse border border-slate-100 dark:border-slate-700"></div>
            ))}
          </div>
        ) : vehicles.length === 0 ? (
          <div className="bg-white dark:bg-slate-800 border border-dashed border-slate-200 dark:border-slate-750 rounded-3xl p-12 text-center shadow-sm">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-450 dark:text-slate-400 mb-4">
              🚗
            </div>
            ...
            <button
              onClick={handleAddClick}
              className="px-5 py-2.5 rounded-xl text-sm font-bold text-white bg-indigo-650 hover:bg-indigo-750 transition"
            >
              Thêm xe ngay
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {vehicles.map((vehicle) => (
              ...
              >
                ...
                <div className="flex justify-between items-center pt-2 border-t border-slate-100 dark:border-slate-700/60 mt-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-3.5 py-2 rounded-xl text-xs font-bold text-white bg-indigo-650 hover:bg-indigo-700 transition"
                    >
                      Cập nhật km
                    </button>
                    <button
